...
*/		
		...
		_app_pallete.menuAction = @selector(openWithApplication:);
		_app_pallete.target = self;		
		[_app_pallete addToView:content_view];
		
		_toolbar_view = [[SimpleViewerToolbarView alloc] initWithFrame:NSZeroRect];
			// not using at 2010-12-06
			//		[content_view addSubview:_toolbar_view];
	}
	return self;
}
...
- (void) dealloc
{
	...
	[_capture_icon_bar_controller release];
	[super dealloc];
}
...
-(void)awakeFromNib
{
	...
	_app_pallete.arrayController = _app_array_controller;

	// must be set at this place, because _app_controller will be set this timing
	_capture_icon_bar_controller = [[CaptureIconBarController alloc] initWithSuperView:[_panel contentView] appController:_app_controller];
	[_capture_icon_bar_controller setFlipped];
...
- (void)layoutViews
{
	...
	NSRect frame_background_view = frame_content_view;

		//	frame_background_view.size.height -= BUTTON_BAR_HEIGHT + METALINE_HEIGHT + TOOLBAR_HEIGHT + TOOLBAR_MARGIN_Y;
	frame_background_view.size.height -= BUTTON_BAR_HEIGHT + METALINE_HEIGHT;
	frame_background_view.origin.y += BUTTON_BAR_HEIGHT + METALINE_HEIGHT;
	[_background_view setFrame:frame_background_view];

	
	NSRect frame_image_view = frame_background_view;
	...
	NSSize cellSize = [ApplicationButtonCell size];
	frame_image_view.size.width -= cellSize.width*2;
	frame_image_view.origin.x += cellSize.width;
	...
	[_app_pallete setDisplayFrame:frame_background_view];
	[_app_pallete updateLayout];

	NSRect frame_toolbar_view = frame_content_view;
	frame_toolbar_view.origin.y = frame_toolbar_view.size.height - TOOLBAR_HEIGHT;
	frame_toolbar_view.size.height = TOOLBAR_HEIGHT;
	[_toolbar_view setFrame:frame_toolbar_view];
	
	[_capture_icon_bar_controller setSuperViewFrame:frame_background_view];
}
...
#pragma mark -
#pragma mark NSWindowDelegte for SimpleViewerPanel
...
- (void)showFile:(NSString*)filename isFitToImage:(BOOL)is_fit withDirection:(int)direction
{
	...
	if (is_fit) {
		if (_is_init_panel_position) {
			NSString* frameString = [UserDefaults valueForKey:UDKEY_VIEWER_WINDOW_POSITION];
			if (frameString) {
...
- (IBAction)clickedCopyFilename:(id)sender
{
	NSPasteboard *pb = [NSPasteboard generalPasteboard];
	[pb declareTypes:[NSArray arrayWithObject:NSStringPboardType] owner:self];
	[pb setString:_filename forType:NSStringPboardType];
	[_app_controller showMessage:NSLocalizedString(@"CopiedFilename", @"")];
}
...
- (void)resetPosition
{
	[self adjustPanelFrameWithContentSize:[self minimumContentSize]];
	NSString* frameString = NSStringFromRect([_panel frame]);
