...
#define TB_MARGIN_HEIGHT	2.0
#define TB_SIZE_MARGIN	3
...
-(id)initWithFrame:(NSRect)frame vertical:(BOOL)vertical
{
...
//	[image2 setFlipped:YES];
	...
	if (buttons && [buttons count] > 0) {
		...
		frame.size.width += _size_margin;
		frame.size.height += _size_margin;
	}
	...
	if (buttons) {
		...
		if (_is_vertical) {
			_offsetY += frame.size.height + TB_MARGIN_HEIGHT;		
			new_size.height = _offsetY;
			if (new_size.width < frame.size.width) {
				new_size.width = frame.size.width;
			}
		} else {
...
- (NSString *)view:(NSView *)view stringForToolTip:(NSToolTipTag)tag point:(NSPoint)point userData:(void *)userData
{
	int i;
	for (i=0; i < [_list count]; i++) {
		ThinButton* button = [_list objectAtIndex:i];
		if (NSPointInRect(point, [button frame])) {
			if (_is_vertical) {				
				button = [_list objectAtIndex:[_list count]-i-1];
			}
...
-(void)setButtonBarWithFrame:(NSRect)frame
{
	...
	// (4) adjust origin.y
	if (p.y - SBBF_MARGIN < frame.origin.y) {
		p.y = frame.origin.y - BUTTON_OFFSET;
	}
	if (p.y + button_size.height + SBBF_MARGIN > view_size.height) {
		p.y = frame.origin.y + frame.size.height  - button_size.height + BUTTON_OFFSET;
...
- (void)recalturateFrame
{
	[self setButtonBarWithFrame:self.frame];
}
...
- (void)setSizeMargin:(CGFloat)sizeMargin
{
	_size_margin = sizeMargin;
}
- (void)setAutoLayout:(BOOL)autoLayout
{
	_auto_layout = autoLayout;
}
