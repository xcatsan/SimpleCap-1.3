...
- (id)initWithAppController:(AppController*)appController
{
	...
	if (self) {
		...
		_option_view = [[CaptureOptionView alloc] initWithFrame:frame];
		[_window setContentView:_view];		
		[_view addSubview:_option_view];
...
- (void)_saveImage:(CGImageRef)cgimage withMouseCursorInRect:(NSRect)rect offset:(NSSize)offset imageFrame:(NSRect)image_frame
{
	if (cgimage) {
		...
		BOOL is_showing_viewer = [[UserDefaults valueForKey:UDKEY_VIEWER_SHOW_AFTER_CAPTURED] boolValue];
		...
		if (_copy_flag) {
			...
			// Save image file
			// save to file
			_last_file_path = [_file_manager saveImage:bitmap_rep];
			if (!_continuous_flag && is_showing_viewer) {
				[_app_controller openViewerWithFile:_last_file_path];
			}
			[_app_controller showMessage:[_last_file_path lastPathComponent]];
		}
...
- (CGWindowID)windowIDOfTimer
{
	return [_timer_controller windowID];
}
